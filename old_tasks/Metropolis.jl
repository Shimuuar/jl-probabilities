...
# ╔═╡ 604e8def-da02-4bd1-a173-0c200f47a0a0
using Random

# ╔═╡ 3723fb40-05bd-4cf7-8bf6-96df55f59a0a
begin
	using Plots
	plotly()
end;
...
function sample_metropolis!(A, pdf, seed, randgen; randgen_params...)
	A[1] = seed
	ε = nextfloat(zero(pdf(A[1])))
	for i ∈ 2 : length(A)
		candidate = randgen(; randgen_params...) + A[i-1]
		acceptance_probability = pdf(candidate) / pdf(A[i-1]) # (pdf(candidate) + ε) / (pdf(A[i-1]) + ε) #
								  # чтобы никогда не происходило деления на 0
		if 1 < acceptance_probability || rand() < acceptance_probability
			A[i] = candidate
		else
			A[i] = A[i-1]
		end
...
function sample_metropolis(pdf, seed, randgen, N, T::Type = Float64; randgen_params...)
	A = Array{T}(undef, N)
	sample_metropolis!(A, pdf, seed, randgen; randgen_params...)
end

# ╔═╡ 95312f59-e035-4716-a99c-fe2d6ca86079
begin
	histogram(
		sample_metropolis(cosine_pdf, 0, () -> 2randn(), 10000),
		normalize = :pdf,
		bins = 40
	)
	plot!(x, cosine_pdf.(x) ./ 4π)
end

# ╔═╡ b83229ca-6fc2-4120-b0cf-63550416c9c3
plot(
	sample_metropolis(cosine_pdf, 0, () -> 2randn(), 2000)
)

# ╔═╡ 4ef206fe-79f0-4b65-93a2-c50b7d59df45
md"Если сделать дисперсию поменьше, то марковской цепи тяжело пересечь перемычку:"

# ╔═╡ 4624d962-7d9a-425c-96df-153474f99da7
begin
	histogram(
		sample_metropolis(cosine_pdf, 0, () -> 0.1randn(), 10000),
		normalize = :pdf,
		bins = 40
	)
	plot!(x, cosine_pdf.(x) ./ 4π)
...
# ╔═╡ 41e31fba-01d6-4447-a375-f2ac03bdfeef
plot(sample_metropolis(cosine_pdf, 0, () -> 0.1randn(), 2000))

# ╔═╡ fb988f95-a125-45e2-b193-9dec4ca4dfd1
md"Если сделать дисперсию побольше, то алгоритм часто топчтется на одном месте:"

# ╔═╡ db95d1b8-9ceb-4e49-bbd5-eef2c4260e2c
begin
	histogram(
		sample_metropolis(cosine_pdf, 0, () -> 1000randn(), 10000),
		normalize = :pdf,
		bins = 40
	)
	plot!(x, cosine_pdf.(x) ./ 4π)
