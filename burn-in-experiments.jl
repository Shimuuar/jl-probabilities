...
# ╔═╡ e7095229-c166-4728-bef2-0f9f10fef24b
Ω = Ω_with_bc[2]
...
test_f(x) = exp(-(x-0.3)^2 * 15) + exp(-(x+0.3)^2 * 15)
...
# ╔═╡ c446d958-e338-4e5e-a9f7-5bc1e04e84bf
...
α_true = splined_f.coefs' * Ω * splined_f.coefs
...
@model function model(
	...
	# coefs ~ MvNormalCanon(n / α .* Matrix(Ω)) # если мы хотим, чтобы α = ∫|f''|²
	
	y_without_noise = kernel_matrix * coefs
	# y_without_noise ~ MvNormal(kernel_matrix * inv(Matrix(Ω)) * kernel_matrix')
	# # жаль K * Ω⁻¹ * K - вырожденная

	σ_noise ~ FlatPos(0.)
	# σ_noise = σ_noise_true
...
# ╔═╡ d03dab6e-bd33-43b5-ad09-f2cc528a8a2d
mean_coefs' * Ω * mean_coefs

# ╔═╡ 41801b76-3e01-42be-9717-bb1c7eacdff5
α_true
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "6b9f6eafe5bb7d80a432a6a0f84b21b9d870c492"
