...
# ╔═╡ 4f2776b2-0609-4b93-8787-7cab2bc5152e
begin
	using Meshes
	using MeshViz
	import WGLMakie
...
# ╔═╡ b42f3e83-11f3-4cf8-b9c4-f3c21abbf90d
using Roots

# ╔═╡ ce3d661d-a23a-4790-aac9-982e737d830f
begin
	using Plots
	import PlotlyJS
	plotlyjs()
...
# ╔═╡ 1afa4338-e900-467f-9af9-d750fbad0cf3
using Trixi

# ╔═╡ d806eb2a-1572-4f62-97df-fe46504375b0


# ╔═╡ 22607435-8c8f-422f-8241-5a3a8438865a
md"## Meshes "

# ╔═╡ 9e45f252-7627-401b-aa85-155cc4b19209
s = Sphere((0,0,0), 1)

# ╔═╡ 419b4390-1162-4d21-ae29-a9dfcc735832
m = simplexify(discretize(s, RegularDiscretization(10)))

# ╔═╡ c1b95084-94b8-4802-9783-1d3b0a413b29
viz(m, showfacets = true)

# ╔═╡ d6d68156-3b7a-457e-95cf-758689a6bf65
u = first(faces(m, 2))

# ╔═╡ 8b3e2d37-594c-4380-b508-f4cdab7e94cf
Vec(coordinates(vertices(u)[2]))

# ╔═╡ b9a6a351-68f3-413e-a9eb-737b7c8139e8
normal(u)

# ╔═╡ 6803a149-6866-461f-98c7-c454b372ecaf
p = vertices(u)[1]

# ╔═╡ b2f4453b-c56e-4950-aa54-59a32b4ab79c
coordinates(p) ⋅ normal(u)

# ╔═╡ 6241f247-7b1b-44fc-bb9f-d99ce0b32427
area(u)

# ╔═╡ 880e1b3e-2a3f-4182-aba4-b00e50647a51
SimpleMesh(vertices(m), topology(m))

# ╔═╡ 0df723fc-08df-4a64-b378-6a7578c5d2b3
coordinates(p) * 2.

# ╔═╡ 6eb27277-6300-420c-9650-bd2154eec91f
function stretch(mesh, by)
	new_points = [Point(coordinates(p) .* by) for p ∈ vertices(mesh)]
	return SimpleMesh(new_points, topology(mesh))
end

# ╔═╡ e5bbbc0d-2f4c-40fb-8060-6217656990cc
m2 = stretch(m, [0.5, 1, 2])

# ╔═╡ a12dae24-dd66-475a-8341-1a02a851f9b3
viz(m2, showfacets = true)

# ╔═╡ 2a03175a-09ab-4ead-a7b1-490d6173175d
md"## ``\Omega``-потенциал"

# ╔═╡ 76e2224c-422a-43cb-9492-16a5ea5573e6
mass_quotient = 0.5

# ╔═╡ 3a1f731d-1fa7-46fe-ba5c-8bd56297d030
lagrange1_x = LagrangePoint_X(mass_quotient)

# ╔═╡ 6abaead3-aa9c-489b-b2a0-06e9a7a7bae6
LagrangePoint_X_textbook(mass_quotient)

# ╔═╡ 1712cbe8-4371-4681-b0fc-0d36ea2cf1bc
begin
	xx = 0.1 : 0.1 : 2
	plot(xx, LagrangePoint_X_textbook.(xx), label = "как в книге")
	plot!(xx, LagrangePoint_X.(xx), label = "Простая формула")
end

# ╔═╡ 76141c50-44ca-46b1-9e04-11ede258e3fa
Ω0 = Ω_critical(lagrange1_x, mass_quotient)

# ╔═╡ f124d04f-b130-4cb9-9604-c5b81c68ba94
roche_r(Ω0, lagrange1_x, mass_quotient, Point(0.8, 0.6, 0))

# ╔═╡ 0ba0cf62-e98c-454a-888e-4defab6a1e76
new_m = TransformToRocheLobe(mass_quotient)(m)

# ╔═╡ b572265b-e5bc-47d9-8096-c3a40076d1fb
viz(new_m, showfacets = true)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "469c61e6b69a0c561ec49e0db8816056ca202873"
...
git-tree-sha1 = "91ee688182fffe47b55509c8d999416b8100ae2b"
uuid = "e4f4c7b8-17cb-445a-93c5-f69190ed6c8c"
version = "0.2.1"

[[deps.HOHQMesh_jll]]
deps = ["Artifacts", "CompilerSupportLibraries_jll", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "cf6f8bd5feff45306cbbd25cc5259accd6101a9d"
...
deps = ["Artifacts", "CompilerSupportLibraries_jll", "JLLWrappers", "LazyArtifacts", "Libdl", "MPIPreferences", "TOML"]
git-tree-sha1 = "788777fb9fdf89115a85e16fd826dcd37c974b46"
...
git-tree-sha1 = "c6e3b867162f25a7214ab083569387694bdcdd95"
...
[[deps.Octavian]]
deps = ["CPUSummary", "IfElse", "LoopVectorization", "ManualMemory", "PolyesterWeave", "Requires", "SnoopPrecompile", "Static", "StaticArrayInterface", "ThreadingUtilities", "VectorizationBase"]
git-tree-sha1 = "f73041af8b18e6e098a93eeffb135db324d1ebf4"
...
git-tree-sha1 = "c3974daa39c95e2b3f92bba3226e4d85ef7e2430"
...
git-tree-sha1 = "5957f6a65b3bb31004a753402d9da24b264244cd"
...
git-tree-sha1 = "44b5f41a467a418f2cd94429f271d1b06f857bce"
...
[[deps.StartUpDG]]
deps = ["ConstructionBase", "FillArrays", "HDF5", "HOHQMesh", "Kronecker", "LinearAlgebra", "NodesAndModes", "PathIntersections", "Printf", "RecipesBase", "RecursiveArrayTools", "Reexport", "Requires", "Setfield", "SimpleUnPack", "SparseArrays", "StaticArrays", "Triangulate", "WriteVTK"]
git-tree-sha1 = "1152e270c8597af56d4f81794087ff5a2229b0f3"
...
git-tree-sha1 = "6cdf94406410fa005b2646fa652c6d159cd8356b"
...
deps = ["ArgCheck", "AutoHashEquals", "FFTW", "InteractiveUtils", "LinearAlgebra", "LoopVectorization", "MuladdMacro", "PolynomialBases", "RecursiveArrayTools", "Reexport", "Requires", "SciMLBase", "SimpleUnPack", "SnoopPrecompile", "SparseArrays", "StaticArrayInterface", "StaticArrays", "Unrolled"]
git-tree-sha1 = "3e3ce6710fa8b18d3ac4fd062aebd3d0036fc1b5"
...
[[deps.Trixi]]
deps = ["CodeTracking", "ConstructionBase", "EllipsisNotation", "FillArrays", "ForwardDiff", "HDF5", "IfElse", "LinearAlgebra", "LinearMaps", "LoopVectorization", "MPI", "MuladdMacro", "Octavian", "OffsetArrays", "P4est", "Polyester", "Printf", "RecipesBase", "Reexport", "Requires", "SciMLBase", "Setfield", "SimpleUnPack", "SparseArrays", "StartUpDG", "Static", "StaticArrays", "StrideArrays", "StructArrays", "SummationByPartsOperators", "TimerOutputs", "Triangulate", "TriplotBase", "TriplotRecipes"]
git-tree-sha1 = "b80b52581d588015ca53cb8caa38c44e3c717d37"
...
git-tree-sha1 = "1f42b49f2b6ec5245a399e48951a72cdb40bd5cb"
uuid = "33b4df10-0173-11e9-2a0c-851a7edac40e"
version = "0.2.24"

[[deps.VectorizedStatistics]]
deps = ["LoopVectorization", "SnoopPrecompile", "Static"]
git-tree-sha1 = "d9b243e2cc5737ab5b6c79c1201429d46c1fae8d"
